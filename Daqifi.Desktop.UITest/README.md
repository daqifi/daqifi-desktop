...
It covers the following end-to-end workflows plus a launch smoke test:
...
| `ManualSerialConnectTests.*` (issue #524) | **Manual USB** tab (typed COM port). A nonexistent port (`COM250`) surfaces the inline red error, **keeps the dialog open**, connects nothing, and logs only a `WARN` — asserting the NLog log has **no** error-level (Sentry) line for the rejected port; the inline error **clears** when the field is edited; and a real enumerated port still **connects** end-to-end (pre-validation does not block valid ports). **USB.** |
...
| `FriendlyNameTests.SetFriendlyName_PersistsAcrossReconnect` (issue #83) | Reads the drawer's **SERIAL** value (non-empty, proving the SN is shown). Sets a friendly name via the drawer's **NAME** field + **SAVE NAME** button, asserts the **"Name saved"** confirmation appears, then **disconnects and reconnects the same device**, briefly streams, and asserts the NAME field shows the same value once the device reports it in a streaming frame — proving the name round-tripped through `SYSTem:DEVice:NAME` + `:SAVE` and actually persisted to firmware NVM, not just the in-session `FriendlyName` property. Client-side validation (empty/too-long/illegal-char names) is covered by `AbstractStreamingDeviceTests` unit tests, not re-verified here. **USB or WiFi.** |
...
| `LoggingSessionTests.ViewLoggedSession_LoadsStoredSessionOntoLoggedDataPlot` | **Logged-session viewer** end-to-end (issue #592). Complements the create/delete scenario by exercising the path that loads a *stored* session from SQLite and renders it on the Logged Data plot + minimap (`DatabaseLogger.DisplayLoggingSession` → the extracted `SessionDataRepository` read path). Runs a brief session to produce a stored row, then loads it onto the plot via the row's invisible `DisplaySessionButton` invoke-seam (the user-facing trigger is a whole-row `MouseBinding` with no InvokePattern, and out-of-process physical row clicks are unreliable, so the row template carries an invisible Button bound to the same `DisplayLoggingSessionCommand`). Asserts black-box that the viewer actually loaded it: the invisible `LoggedSessionStatsText` hook reports a **non-zero displayed-session sample count** (0 until `DisplayLoggingSession` reads the session and populates the plot, see gotcha #18) and the app logged **no** `Failed in DisplayLoggingSession`. Self-cleans by deleting the session it created. |
...
| `CsvExportTests.ExportLoggedSession_ProducesValidCsv` / `…ExportAllLoggedSessions_ProducesValidCsv` | Run a short logging session, then export it to CSV — once via the per-session **EXPORT** button and once via **EXPORT ALL** — through the `DAQIFI_TEST_EXPORT_PATH` hook (**no `SaveFileDialog`**, see below). Read the produced CSV back from disk (black box) and validate it: header is `Time` + one `Device:Serial:Channel` column per channel with no formula-injection prefix; every row has the header's field count (RFC 4180 consistency via a strict quote-aware parser); the time column parses as a round-trip timestamp and is non-decreasing; every value cell is a finite invariant-culture number; and the **value-cell count equals the session's distinct (timestamp, device, serial, channel) position count** (each position → one cell), proving no rows were dropped, duplicated, or corrupted. The oracle is read out-of-process from the app's test-mode finalize log lines (`Persisted sample count N…` + `Persisted distinct sample positions D…`); positions — not raw samples — because the device occasionally duplicates a frame's timestamp and exporters collapse those duplicates (gotcha #20). |
| `StreamRestartTests.RestartStreamingAfterGap_TimeAxisAnchorsOnNewSession` | Stream restart **time-axis anchoring** (issue #573). Runs a session, stops, waits a deliberate ~8 s gap (the test *stimulus*: the device's free-running counter keeps running and the device holds the stopped session's final frame in its transmit path, emitting it first on restart), restarts, and asserts — via the plot-stats hook's `firstx`/`lastx` fields — that the new session's axis **starts at a non-negative X near zero** and **spans only the time actually streamed**, not the stop-to-start gap. On unfixed builds the leftover frame anchors the baseline, putting real data at `+gap` ms (or ~−73 s across a counter wrap). Self-cleans its two sessions. **USB or WiFi.** |
...
| `DigitalOutputTests.DigitalOutput_DirectionAndDriveRoundTrip` | **Digital output** end-to-end (issue #663). Flips a digital channel to OUTPUT through the channel-settings drawer, commands it HIGH there, asserts the tile re-shelves into the DIGITAL OUT section with the commanded state on its per-tile drive toggle, drives the pin LOW/HIGH from the tile toggle, and returns the channel to INPUT (toggle leaves the tree). Every direction/state change delegates to Core's `SetDioDirection`/`SetDioValue` against the live device; the NLog log is asserted free of the wrapper's `Failed to …`/`Ignored DIO` lines, so the commands really executed rather than being swallowed. Targets the drawer by opening the **last** tile's gear (sections render AI → DI → DO, so with factory input directions that is the highest-numbered digital channel; the drawer header name is asserted). **USB or WiFi.** |
| `PwmOutputTests.PwmOutput_EnableAdjustDisableRoundTrip` | **PWM output** end-to-end (issue #664). Asserts capability gating both ways — the highest-numbered digital channel (not PWM-capable) renders **no** PWM section in its settings drawer, while `DIO7` (PWM-capable per Core's board mask) does — then stages a 45 % duty and the **device-wide** frequency (the field commits on focus loss so half-typed values are never commanded), enables PWM, and asserts the drawer **suppresses DIRECTION and OUTPUT STATE** while PWM runs (the hardware ignores those writes). After closing the drawer it asserts the tile **re-shelved into DIGITAL OUT showing `PWM 45%`** on its value line with the per-tile drive toggle suppressed, changes the duty **live** to 80 % (tile follows), disables PWM, and asserts the DIRECTION controls return and the duty readout leaves the pane. Every change delegates to Core's `SetPwmEnabled`/`SetPwmDutyCycle`/`SetPwmFrequency` in the documented duty → frequency → enable order; the NLog log is asserted free of the wrapper's `Failed to …`/`Ignored …` PWM lines. **USB or WiFi.** |
| `ChannelMathTests.ChannelMath_AppliesXTimes10Scaling_ScalesDisplayedValue` / `…InvalidExpression_DisablesScaling_RawValuesFlow` | **Channel math (scaling expression)** end-to-end (issue #556). Enables an analog channel and starts a brief logging session (the device only streams — and so only updates a tile's displayed value — while a session is active), then drives the channel-settings drawer's SCALING section: switches it on, sets the expression to `x * 10`, and asserts the displayed value is ~10x its pre-scaling baseline (averaged over several readings to smooth ADC noise), with no INVALID EXPRESSION warning. The companion test sets a deliberately invalid expression (`x *`) and asserts the drawer shows INVALID EXPRESSION while the displayed value keeps tracking the raw (unscaled) baseline — proving `HasValidExpression == false` disables scaling without freezing or corrupting the tile. Both self-clean the logging session they create. **USB or WiFi.** |
...
The companion oracle is a pair of log lines the app writes when a session finalizes
(`LoggingManager.PersistSessionSampleCount`, emitted after all buffered samples are flushed):
**`Persisted sample count N for session S`** (always) and
**`Persisted distinct sample positions D for session S`** (test mode only, where a *position* is
a distinct `(timestamp, device, serial, channel)` tuple). The harness asserts the exported CSV
holds exactly **`D`** value cells — not `N`, because the device occasionally re-emits a frame
with a duplicated timestamp tick and every exporter collapses same-position duplicates into one
cell, last value wins (see gotcha #20). `D == N` whenever no duplicated frame occurred. The
`DISTINCT` query can cost ~a second on million-sample sessions, which is why the `D` line is
gated on `DAQIFI_TEST_MODE` and never runs in production.
...
| Device-drawer **DISCONNECT** button (ACTIONS section; tears the device down — issue #559) | `DisconnectSelectedButton` | `View/Prototype/DevicesPanePrototype.xaml` |
| Sample-frequency slider (in the device flyout) | `SamplingFrequencyInput` | `View/Prototype/DevicesPanePrototype.xaml` |
| Device drawer SERIAL info-card value / NAME field + SAVE NAME button (issue #83) | `SerialNumber` / `FriendlyNameInput` / `SaveFriendlyNameButton` | `View/Prototype/DevicesPanePrototype.xaml` |
| Connection dialog tabs (`ConnTab_Manual` is the Manual **WiFi** tab) | `ConnTab_Wifi` / `ConnTab_Manual` / `ConnTab_Serial` | `View/ConnectionDialog.xaml` |
...
| Per-tile channel settings gear (one per tile, shared literal id — target by position, cf. `ExportSessionButton`) | `ChannelSettingsButton` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Channel settings drawer: header name / close | `ChannelSettingsName` / `CloseChannelSettingsButton` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Channel settings drawer: INPUT/OUTPUT direction segmented toggle (IsChecked-driven, gotcha #12-safe) | `DirectionInputRadio` / `DirectionOutputRadio` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Channel settings drawer: OUTPUT STATE segmented toggle (digital + output only) | `OutputStateLowRadio` / `OutputStateHighRadio` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Digital-output tile drive switch (one per DO tile; TogglePattern reads and drives the commanded state; absent while PWM runs) | `OutputDriveToggle` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Channel settings drawer: PWM mode toggle (rendered only on PWM-capable channels — a null lookup is the capability signal) | `PwmModeToggle` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Channel settings drawer: PWM duty slider (RangeValuePattern; binding commits after a 500 ms `Delay`) | `PwmDutyCycleSlider` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Channel settings drawer: device-wide PWM frequency field (commits on **focus loss** — shift focus to another drawer control after `ValuePattern.SetValue`) | `PwmFrequencyInput` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Channel settings drawer: SCALING section (analog only, issue #556) — on/off toggle, expression box (disabled until the toggle is on; commits after a 300 ms `Delay`), and the INVALID EXPRESSION warning (Collapsed — and so absent from the UIA tree — unless scaling is on and the expression is invalid) | `ScalingActiveToggle` / `ScaleExpressionInput` / `InvalidExpressionText` | `View/Prototype/ChannelsPanePrototype.xaml` |
| Per-tile value line (one per tile that shows a value; lets the harness read e.g. `PWM 45%`) | `ChannelTileValueText` | `View/Prototype/ChannelsPanePrototype.xaml` |
...
17. **Destructive confirmations use a dark *in-pane* overlay, not a MahApps modal.** Delete actions
    ...
    **both** style variants (accent for non-destructive, danger/red for destructive); their
    ...
    `LoggingSessions` collection (the view). Since the SessionDataRepository extraction (#592),
    `DbLogger.DeleteLoggingSession` rethrows on failure, so the view-model removes the row only when
    the delete succeeds. DB-level deletion is still asserted separately from the app's NLog lines
    (`DeleteLoggingSession completed` present, `Failed in DeleteLoggingSession` absent) as independent
    confirmation that the rows really left the database.
...
18. **The live plot is an OxyPlot canvas with no per-point UIA elements — assert on a stats hook,
    ...
    `"series=N;points=M;nonfinite=K;last=V;min=A;max=B;firstx=X;lastx=Y"` (`firstx`/`lastx` are the
    axis-X span in elapsed ms — the time-axis anchor, used by `StreamRestartTests` for issue #573)
    recomputed on the plot's own once-a-second
...
20. **The device occasionally duplicates a frame's timestamp — don't assert CSV cells against the
    raw persisted sample count.** Mid-session, DAQiFi hardware sometimes re-emits a frame whose
    timestamp tick equals the previous frame's (the same hardware behavior family addressed for
    plotting in #582 and stream start in #584; observed live: a 5,824-sample, 16-channel session
    with only 363 distinct ticks — one fully duplicated frame). Both the legacy CSV exporter and
    `Daqifi.Core`'s `CsvExporter` **intentionally collapse same-(timestamp, device, serial,
    channel) duplicates into one cell, last value wins** (byte-identical output verified between
    the two on such a session), so the CSV legitimately holds `channelCount` fewer cells per
    duplicated frame than the session persisted samples. An exact-equality assertion against
    `Persisted sample count N` is therefore *data-flaky*: it fails on no product defect whenever
    the device happens to duplicate a frame during the run. The harness instead asserts against
    the test-mode-only **`Persisted distinct sample positions D for session S`** finalize line
    (`WaitForDistinctSamplePositions`), which counts distinct positions — exactly one CSV cell
    each, with `D == N` when no duplication occurred. If an export/count assertion fails by
    exactly one frame's worth of cells, check distinct ticks vs sample count before suspecting
    the exporter.
